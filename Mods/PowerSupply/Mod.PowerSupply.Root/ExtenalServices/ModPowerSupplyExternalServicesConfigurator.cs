...
using Mod.PowerSupply.Services.Consumers;
using Serilog;
using Serilog.Sinks.GrafanaLoki;

namespace Mod.PowerSupply.Root.ExtenalServices;

public class ModPowerSupplyExternalServicesConfigurator
{
    private readonly IServiceCollection _services;
    private readonly PowerSupplyEnvironmentContext _productEnvironmentContext;
    private readonly WebApplicationBuilder _builder;

    public ModPowerSupplyExternalServicesConfigurator(WebApplicationBuilder builder, PowerSupplyEnvironmentContext productEnvironmentContext)
    {
    ...
    public void Configure()
    {
        ...
        _services.AddAutoMapper(typeof(GetAllPowerSupplysQuery).Assembly); 
        // _services.AddMediatR(typeof(GetAllPowerSupplysQuery).Assembly);
        _services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(GetAllPowerSupplysQuery).Assembly));
    ...
    public void ConfigureLogging()
    {
        ...
        Log.Logger = new LoggerConfiguration()
            ...
            .WriteTo.GrafanaLoki(
                "http://localhost:3100",
                credentials,
                new Dictionary<string, string>() { { "app", "Serilog.Sinks.GrafanaLoki.PowerSupplyWebApi" } }, // Global labels
